...
p = 0.3
# Use mean imputation
r_base3 <- SimEval(data, n.sim = n.sim, guess = TRUE, guess.type = "mean")
r_base3
# all lmFuns
...
system.time(sim_3 <- 
              foreach (i = seq_along(lmfuns)) %dopar% 
              (SimEval(data, p = p, n.sim = n.sim, method = lmfuns[i], verbose = FALSE))
)
)
...
sim_conv <- sapply(paste("sim", lmfuns, sep = "_"), function(i) unlist(eval(as.name(i))$conv))
sim_avconv <- apply(sim_conv, 2, median)
s# combine to base
sim_error <- data.frame(cbind(r_base$error, sim_error))
colnames(sim_error) <- c("mean", lmfuns)

mean_nrmse <- apply(sim_error, 2, mean)
sd_nrmse <- apply(sim_error, 2, sd)

# wilcox.test
test_nrmse <- sapply(sim_error[, -1], function(i) wilcox.test(x = i, y = sim_error[, 1],
                                                              alternative = "less")$p.value)


all_test <- matrix(0, ncol(sim_error), ncol(sim_error))
for (i in 1:ncol(sim_error)) {
  for (j in 1:ncol(sim_error)) {
    x2 <- sim_error[, i]
    y2 <- sim_error[, j]
    all_test[i, j] <- wilcox.test(x2, y2)$p.value  
  }
}
colnames(all_test) <- colnames(sim_error)
stargazer(all_test, digits = 2)
